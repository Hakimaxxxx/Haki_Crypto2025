...
def fetch_block_transactions(block_number):
    url = f"https://api.bscscan.com/api?module=proxy&action=eth_getBlockByNumber&tag={hex(block_number)}&boolean=true"
    r = requests.get(url, timeout=10)
    data = r.json()
...
def show_bnb_whale_alert_realtime(min_value_bnb=1000, num_blocks=5):
    st.markdown("""
...
def background_whale_alert_scanner(min_value_bnb=500, num_blocks=5, interval_sec=300):
    while True:
        try:
            ...
            for block_num in range(start_block, end_block - 1, -1):
                ...
                for tx in txs:
                    value_bnb = int(tx.get("value", "0"), 16) / 1e18
                    tx_hash = tx.get("hash", "")
